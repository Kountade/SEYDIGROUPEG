        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-base-200 rounded-lg p-4 text-center">
              ...
              <p className="text-2xl font-bold text-base-content">{formatGNF(payroll.gross_salary)}</p>
            </div>
            <div className="bg-success/10 rounded-lg p-4 text-center border border-success/20">
              ...
              <p className="text-3xl font-bold text-success">{formatGNF(payroll.net_salary)}</p>
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-base-200 rounded-lg divide-y divide-base-300">
                <div className="flex justify-between p-3">
                  ...
                  <span className="font-medium">{formatGNF(payroll.base_salary)}</span>
                </div>
                {payroll.performance_bonus > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Prime de performance</span>
                    <span className="font-medium text-success">+ {formatGNF(payroll.performance_bonus)}</span>
                  </div>
                )}
                {payroll.seniority_bonus > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Prime d'ancienneté</span>
                    <span className="font-medium text-success">+ {formatGNF(payroll.seniority_bonus)}</span>
                  </div>
                )}
                {payroll.overtime_amount > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Heures supplémentaires</span>
                    <span className="font-medium text-success">+ {formatGNF(payroll.overtime_amount)}</span>
                  </div>
                )}
                {payroll.transport_bonus > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Indemnité transport</span>
                    <span className="font-medium text-success">+ {formatGNF(payroll.transport_bonus)}</span>
                  </div>
                )}
                {payroll.phone_bonus > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Indemnité téléphone</span>
                    <span className="font-medium text-success">+ {formatGNF(payroll.phone_bonus)}</span>
                  </div>
                )}
                {payroll.other_bonus > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Autres primes</span>
                    <span className="font-medium text-success">+ {formatGNF(payroll.other_bonus)}</span>
                  </div>
                )}
                <div className="flex justify-between p-3 bg-primary/5 font-bold">
                  <span>Total brut</span>
                  <span>{formatGNF(payroll.gross_salary)}</span>
                </div>
              </div>
            </div>
              ...
              <div className="bg-base-200 rounded-lg divide-y divide-base-300">
                {payroll.social_security > 0 && (
                  <div className="flex justify-between p-3">
                    ...
                    <span className="font-medium text-error">- {formatGNF(payroll.social_security)}</span>
                  </div>
                )}
                {payroll.income_tax > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Impôt sur le revenu</span>
                    <span className="font-medium text-error">- {formatGNF(payroll.income_tax)}</span>
                  </div>
                )}
                {payroll.pension_fund > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Fonds de pension</span>
                    <span className="font-medium text-error">- {formatGNF(payroll.pension_fund)}</span>
                  </div>
                )}
                {payroll.health_insurance > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Mutuelle</span>
                    <span className="font-medium text-error">- {formatGNF(payroll.health_insurance)}</span>
                  </div>
                )}
                {payroll.unpaid_leave > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Congé sans solde</span>
                    <span className="font-medium text-error">- {formatGNF(payroll.unpaid_leave)}</span>
                  </div>
                )}
                {payroll.other_deductions > 0 && (
                  <div className="flex justify-between p-3">
                    <span className="text-sm">Autres déductions</span>
                    <span className="font-medium text-error">- {formatGNF(payroll.other_deductions)}</span>
                  </div>
                )}
                <div className="flex justify-between p-3 bg-error/5 font-bold">
                  <span>Total déductions</span>
                  <span className="text-error">- {formatGNF(payroll.gross_salary - payroll.net_salary)}</span>
                </div>
              </div>
            </div>
          </div>
